...
class C_LiftingMotorCtrl_850pro():
    ...
    def LiftMovePos(self, height:int):
        ...
        # print("目标脉冲为: ",pause)
        # if isinstance(self.__motor_msgs, self.motor_msg):
        #     print("当前高度为:" ,self.__motor_msgs.back_lift_height)
        #     print("当前脉冲为:" ,self.__motor_msgs.back_pos)
    ...
    def MotorSetSpeedTime(self, time:int):
        '''
        设定电机速度模式启停时间
        '''
        txlist = [self.__motor_id, 0x06, 0x00, 0x0a, time, time]
        rxdata =  self.__deal_data.DealAllData(txlist, 6, True, self.__port_name)
        print("设定 ",self.__motor_id," 号电机速度模式下启动和停止时间为: ",time, "")
        pass
