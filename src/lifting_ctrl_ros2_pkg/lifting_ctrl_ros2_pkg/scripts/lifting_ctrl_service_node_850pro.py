...
class C_ROS_Server(Node):
    def __init__(self, id:int=None) -> None:
        ...
        初始化节点
        '''
        self.node_name = 'lifting_ctrl_server_node_850pro_' + str(id)
        super().__init__(self.node_name)
        self.get_logger().info("寻找已经存在的节点")
        self.motor_id = id
        self.ctrl = C_LiftingMotorCtrl_850pro(self.motor_id)
        self.check_existing_nodes()
        self.ctrl.MotorConfigInit()
        # self.declare_parameter('motor_id', self.motor_id)
        ...
        self.js_msgs = JointState()
        self.js_pub = self.create_publisher(JointState, 'lifting/joint_states', 1)

        if(self.callBackMode == 0):
            self.js_sub = self.create_subscription(JointState, 'lifting/joint_states_ctrl', self.joint_states_callback_block, 1)
            self.motor_srv = self.create_service(LiftMotorSrv, 'LiftingMotorService', self.SververCallbackBlock)
        elif(self.callBackMode == 1):
            self.js_sub = self.create_subscription(JointState, 'lifting/joint_states_ctrl', self.joint_states_callback_immediately, 1)
    ...
    def PublishMotorMsgs(self) -> None:
        ...
        if isinstance(self.motor_states, C_LiftingMotorCtrl_850pro.motor_msg):
            # 获取当前时间
            self.js_msgs.header.stamp = self.get_clock().now().to_msg()
            # 当前话题电机id
            ...
            self.js_msgs.position = [float(self.motor_msgs.back_height)]
            # 升降柱目标高度
        ...
        try:
            ...
            self.js_pub.publish(self.js_msgs)
    ...
    def SververCallbackImmediately(self, request, response):
        ...
        if(self.callLock == self.last_callLock):
            ...
            # 恒定速度
                ...
                #     f"----------操作电机 {self.motor_id} 最终发送速度数值为"
                #     f"{self.ctrl.ConvertSendRealSpdVal(self.ctrl.LiftSpdConvertToMotorSpd(self.target_speed))}"
                # )
                if isinstance(self.motor_states, C_LiftingMotorCtrl_850pro.motor_msg):
            ...
            # 恒定速度
                ...
                if(self.target_speed < -max_spd): self.target_speed=-max_spd
                # self.get_logger().info(f"----------操作电机 {self.motor_id} 最终发送速度数值为 {self.ctrl.ConvertSendRealSpdVal(self.target_speed)}")
                if isinstance(self.motor_states, C_LiftingMotorCtrl_850pro.motor_msg):
                    # self.get_logger().info(str(self.motor_states.back_speed))
    ...
    def joint_states_callback_immediately(self, msg: JointState):
        '''
        850Pro 服务端回调函数,立刻反馈数值，不等待电机到达位置就反馈
        '''
        self.input_callback_time = time.time()
        ...
        if(self.callLock == self.last_callLock):
            ...
            if len(msg.position) > 0:
                self.mode = 0
                val = int(msg.position[0])
            elif len(msg.velocity) > 0:
                self.mode = -4
                val = int(msg.velocity[0])
            ...
            # 位置模式
            elif(self.mode == 0 and self.init_state):
                self.target_height = self.ctrl.SetTargetHeight(val)
                self.get_logger().info(f"target height: {self.target_height}")
                # 电机是否到达了目标位置，误差小于1认为到达了，反馈1为到达，反馈-1为未到达
                if(abs(self.target_height - self.back_height) < 2):
                    resp = 1
            ...
            # 急停
            elif(self.mode == -2 and self.init_state):
                if isinstance(self.motor_states, C_LiftingMotorCtrl_850pro.motor_msg):
                    if(self.motor_states.back_speed == 0):
                        resp = 1
                else: resp = -1
            # -3 指令用来初始化电机，并清空错误帧率
                ...
                # else: resp = -1
            # 恒定速度
            elif(self.mode == -4 and self.init_state):
                # self.get_logger().info(f"the max speed is: {self.liftTargetSpd}")
                self.ctrl.SetTargetHeight(self.back_height)
            ...
            elif(self.mode == -5 and self.init_state):
                self.target_speed = val
                if(self.target_speed > self.liftTargetSpd): self.target_speed=self.liftTargetSpd
                if(self.target_speed < -self.liftTargetSpd): self.target_speed=-self.liftTargetSpd
                self.target_speed = self.ctrl.LiftSpdConvertToMotorSpd(self.target_speed)
                ...
                if(abs(self.motor_states.back_pos) <= 1):
                    ...
                    resp = -1
            # 恒定速度
            elif(self.mode == -7 and self.init_state):
                # self.get_logger().info(f"the max speed is: {self.liftTargetSpd}")
                self.ctrl.SetTargetHeight(self.back_height)
                self.target_speed = val
                max_spd = self.ctrl.GetMotorMaxSpd()
        ...
        if(self.callLock == self.last_callLock):
            while response == -1:
                ...
                # 恒定速度
                    ...
                    # )
                    if isinstance(self.motor_states, C_LiftingMotorCtrl_850pro.motor_msg):
                        # self.get_logger().info(f"{self.motor_states.back_speed}")
                        if(abs((self.ctrl.CalLiftSpd(self.motor_states.back_speed)) - self.target_speed)<20):
                            response = 1
                    else: response = -1
                ...
                # 恒定速度
                    ...
                    if(self.target_speed > max_spd): self.target_speed=max_spd
                    if(self.target_speed < -max_spd): self.target_speed=-max_spd
                    # self.get_logger().info(f"----------操作电机 {self.motor_id} 最终发送速度为{self.ctrl.ConvertSendRealSpdVal(self.target_speed)}")
                    if isinstance(self.motor_states, C_LiftingMotorCtrl_850pro.motor_msg):
    ...
    def joint_states_callback_block(self, msg: JointState):
        '''
        服务端回调函数，阻塞式，等待电机到达目标位置才会反馈
        '''
        self.input_callback_time = time.time()
